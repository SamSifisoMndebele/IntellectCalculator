...
object BinaryArithmetic {
    ...
    fun String.fillBits(isNegative : Boolean = false): String {
        ...
        return if (isNegative) {
            this.padStart(bits + 4, '0')
        } else {
            this.padStart(bits, '0')
        }
    }
    ...
    fun String.toDecimal(base: Int = 2): Long {
        ...
        for (digit in this.reversed()){
            val parseDouble = digit.toString().toDouble()

            java.lang.Double.isNaN(parseDouble)

            decimal += (parseDouble * base.toDouble().pow(i.toDouble())).toLong()
            i++
        }
