...
import com.avidco.intellectcalculator.ui.expressions.treediagram.graphview.util.Size
import com.avidco.intellectcalculator.ui.expressions.treediagram.graphview.util.VectorF
import java.util.*
import kotlin.math.max
import kotlin.math.min

class TreeLayoutManager @JvmOverloads constructor(private val context: Context,
    private val configuration: TreeConfiguration = TreeConfiguration.Builder()
        .build()
) : GraphLayoutManager() {
    private val mNodeData: MutableMap<Node, TreeNodeData> = HashMap()
    ...
    private val isVertical: Boolean
        get() {
            ...
            return orientation == TreeConfiguration.ORIENTATION_TOP_BOTTOM || orientation == TreeConfiguration.ORIENTATION_BOTTOM_TOP
        }
    ...
    private fun createNodeData(node: Node): TreeNodeData {
        val nodeData = TreeNodeData().apply {
            ancestor = node
        }
    ...
    private fun getNodeData(node: Node): TreeNodeData {
    ...
    private fun positionNodes(graph: Graph) {
        ...
        val needReverseOrder =
            orientation == TreeConfiguration.ORIENTATION_BOTTOM_TOP || orientation == TreeConfiguration.ORIENTATION_RIGHT_LEFT
        ...
        nodes.forEach { node ->
            ...
            if (depth != currentLevel) {
                when (configuration.orientation) {
                    TreeConfiguration.ORIENTATION_TOP_BOTTOM, TreeConfiguration.ORIENTATION_LEFT_RIGHT -> globalPadding += localPadding
                    TreeConfiguration.ORIENTATION_BOTTOM_TOP, TreeConfiguration.ORIENTATION_RIGHT_LEFT -> globalPadding -= localPadding
                }
                localPadding = 0
                currentLevel = depth
            ...
            when (configuration.orientation) {
                TreeConfiguration.ORIENTATION_TOP_BOTTOM -> if (height > minNodeHeight) {
                    val diff = height - minNodeHeight
                    localPadding = max(localPadding, diff)
                }
                TreeConfiguration.ORIENTATION_BOTTOM_TOP -> if (height < localMaxSize.height) {
                    val diff = localMaxSize.height - height
                    node.setPosition(node.position.subtract(VectorF(0f, diff.toFloat())))
                    localPadding = max(localPadding, diff)
                }
                TreeConfiguration.ORIENTATION_LEFT_RIGHT -> if (width > minNodeWidth) {
                    val diff = width - minNodeWidth
                    localPadding = max(localPadding, diff)
                }
                TreeConfiguration.ORIENTATION_RIGHT_LEFT -> if (width < localMaxSize.width) {
    ...
    private fun getOffset(graph: Graph): VectorF {
        ...
        when (configuration.orientation) {
            TreeConfiguration.ORIENTATION_BOTTOM_TOP, TreeConfiguration.ORIENTATION_RIGHT_LEFT -> offsetY =
        ...
        graph.nodes.forEach { node ->
            when (orientation) {
                TreeConfiguration.ORIENTATION_TOP_BOTTOM, TreeConfiguration.ORIENTATION_LEFT_RIGHT -> {
                    offsetX = min(offsetX, node.x)
                    offsetY = min(offsetY, node.y)
                }
                TreeConfiguration.ORIENTATION_BOTTOM_TOP, TreeConfiguration.ORIENTATION_RIGHT_LEFT -> {
                    offsetX = min(offsetX, node.x)
    ...
    private fun getPosition(node: Node, globalPadding: Int, offset: VectorF): VectorF =
            when (configuration.orientation) {
                TreeConfiguration.ORIENTATION_TOP_BOTTOM -> VectorF(
                        node.x - offset.x,
                        node.y + globalPadding
                )
                TreeConfiguration.ORIENTATION_BOTTOM_TOP -> VectorF(
                        node.x - offset.x,
                        offset.y - node.y - globalPadding.toFloat()
                )
                TreeConfiguration.ORIENTATION_LEFT_RIGHT -> VectorF(
                        node.y + globalPadding,
                        node.x - offset.x
                )
                TreeConfiguration.ORIENTATION_RIGHT_LEFT -> VectorF(
                        offset.y - node.y - globalPadding.toFloat(),
                        node.x - offset.x
                )
                else -> {
